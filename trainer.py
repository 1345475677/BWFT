...
class Trainer:
    ...
    def __init__(self, args, seed, metric_keys, save_keys):
        ...
        if args.dataset == 'CIFAR10':
            ...
            Dataset = dataloaders.iIMAGENET_100
            num_classes = 100
            self.dataset_size = [224, 224, 3]
            self.top_k = 1
            ...
            self.dataset_size = [224,224,3]
            self.top_k = 1
        elif args.dataset == 'SKIN40':
            Dataset = dataloaders.iSKIN40
            num_classes = 40
            self.dataset_size = [224,224,3]
            self.top_k = 1
        elif args.dataset == 'PathMNIST':
            Dataset = dataloaders.iPathMNIST
            num_classes = 9
            self.dataset_size = [32,32,3]
            self.top_k = 1
        elif args.dataset == 'OrganSMNIST':
            Dataset = dataloaders.iOrganSMNIST
            num_classes = 11
            self.dataset_size = [32,32,3]
            self.top_k = 1
        else:
    ...
    def train(self, avg_metrics):
        ...
        for i in range(self.max_task):
            ...
            if self.oracle_flag:
                ...
                print("加载数据集前",len(self.train_dataset))
                self.train_dataset.load_dataset(i, train=True)
                self.add_dim = len(task)
                print("加载数据集后",len(self.train_dataset))
            ...
            print("更新缓冲区前",len(self.train_dataset))
            # load dataset with memory
            self.train_dataset.append_coreset(only=False)
            print("更新缓冲区后",len(self.train_dataset))
            # load dataloader
