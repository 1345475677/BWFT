...
from torchvision.datasets.imagenet import parse_train_archive, parse_val_archive, parse_devkit_archive, META_FILE
...
class iDataset(data.Dataset):
    ...
    def append_coreset(self, only=False, interp=False):
        ...
        if self.train and (len_core > 0):
            if only:
                ...
                sample_ind = np.random.choice(len_core, len_core,replace=False)
                print("sample_ind",len(sample_ind))
    ...
    def update_coreset(self, coreset_size, seen):
        ...
        for k in reversed(seen):
            mapped_targets = [self.class_mapping[self.targets[i]] for i in range(len(self.targets))]#将当前任务分片的真实类别映射到整理过的类别 真类->伪类
            locs = (mapped_targets == k).nonzero()[0]#找出第k个伪类的全部loc
...
class iIMAGENET_100(iDataset):
    ...
    def load(self):
        ...
        # load splits from config file
        if self.train or self.validation:
            data_config = yaml.load(open('dataloaders/splits/imagenet-100_train.yaml', 'r'), Loader=yaml.Loader)
        else:
            data_config = yaml.load(open('dataloaders/splits/imagenet-100_test.yaml', 'r'), Loader=yaml.Loader)
        self.data = data_config['data']
        self.targets = data_config['targets']
...
class iSKIN40(iIMAGENET_R):
    ...
    def load(self):
        ...
        # load splits from config file
        if self.train or self.validation:
            data_config = yaml.load(open('dataloaders/splits/skin40_train.yaml', 'r'), Loader=yaml.Loader)
        else:
            data_config = yaml.load(open('dataloaders/splits/skin40_test.yaml', 'r'), Loader=yaml.Loader)
        self.data = data_config['data']
        self.targets = data_config['targets']


class iPathMNIST(iDataset):
    im_size = 28
    nch = 3
    ...
    def load(self):
        ...
        self.data,self.targets=dataset.imgs, np.array(dataset.labels).squeeze(1)


class iOrganSMNIST(iDataset):
    im_size = 28
    nch = 3
    ...
    def load(self):
        ...
        # load splits from config file
        if self.train or self.validation:
            dataset = OrganSMNIST(split="train", download=True)
        else:
            dataset = OrganSMNIST(split="test", download=True)
        self.data,self.targets=dataset.imgs, np.array(dataset.labels).squeeze(1)
        self.data=np.repeat(np.expand_dims(self.data,axis=3),3,axis=3)
...
class feature_dataset(data.Dataset):
    def __init__(self,root,save_distribution=0):
        self.save_distribution=save_distribution
        if save_distribution==0:
            self.feature_list,self.label_list=torch.load(root)#[N,global_bs,dim]
            self.feature_list=torch.cat(self.feature_list,dim=0)#[global_BS,dim]
            self.label_list=torch.cat(self.label_list,dim=0)
        else:
            self.feature_list,self.label_list=torch.load(root)
    def __getitem__(self, index):
        if self.save_distribution==0:
            return self.feature_list[index],self.label_list[index]
        elif self.save_distribution==1:
            mean=self.feature_list[index]["mean"]
            original_mean=self.feature_list[index]["original_mean"]
            std=self.feature_list[index]["std"]
            label=self.label_list[index]
            if random.random()<0.5:#50%的概率使用数据增强
                feature=torch.normal(mean,std)
            else:
                feature=torch.normal(original_mean,std)
            return feature,label
        elif self.save_distribution==2:
            original_mean = self.feature_list[index]["original_mean"]
            label=self.label_list[index]
            sim_index=self.feature_list[index]["sim_index"]
            original_mean2=self.feature_list[sim_index[random.randint(0,4)]]["original_mean"]
            std=original_mean2-original_mean
            feature=original_mean+std*random.random()*2#std*(0~2)
            return feature,label
        elif self.save_distribution==3 or self.save_distribution==4 or self.save_distribution==5:
            mean = self.feature_list[index]["mean"]
            std = self.feature_list[index]["std"]
            label = self.label_list[index]
            feature = torch.normal(mean, std)
